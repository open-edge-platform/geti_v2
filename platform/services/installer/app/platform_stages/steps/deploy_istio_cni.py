...
from constants.charts import ISTIO_CNI_CHART
...
FULL_VALUES_FILE_NAME = str(os.path.join(PLATFORM_INSTALL_PATH, ISTIO_CNI_CHART.values_file))


def deploy_istio_cni_chart(config: InstallationConfig | UpgradeConfig, charts_dir: str = PLATFORM_INSTALL_PATH) -> None:
    """
    Method used to deploy Istio CNI chart
    """
    if not is_feature_flag_enabled(FeatureFlag.AMBIENT_MESH):
        return
    data = {"registry": config.image_registry.value}
    save_jinja_template(
        source_file=ISTIO_CNI_CHART.values_template_file, data=data, destination_file=FULL_VALUES_FILE_NAME
    )
    values_file = [FULL_VALUES_FILE_NAME]

    try:
        chart_version = get_target_product_build() if config.lightweight_installer.value else None
        chart_dir = str(os.path.join(charts_dir, ISTIO_CNI_CHART.directory))
        if chart_version:
            pull_chart(chart_version, chart_dir)
        upsert_chart(
            name=ISTIO_CNI_CHART.name,
            version=chart_version,
            chart_dir=chart_dir,
            namespace=ISTIO_CNI_CHART.namespace,
            values=values_file,
            sets="profile=ambient,global.platform=k3s",
        )
