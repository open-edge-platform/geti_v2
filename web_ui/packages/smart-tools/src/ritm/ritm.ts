...
import { Point, RegionOfInterest, Shape, ShapeType } from '../shared/interfaces';
import { approximateShape, concatFloat32Arrays, isPolygonValid, loadSource, stackPlanes } from '../utils/utils';
import { sessionParams } from '../utils/wasm-utils';
import { MainModelResponse, Models, RITMContour, RITMPoint, TEMPLATE_SIZE } from './interfaces';
...
export class RITM {
    ...
    async load() {
        ...
        this.models = {
            main: await this.loadModel(new URL('./models/main.onnx', import.meta.url).toString()),
            preprocess: await this.loadModel(new URL('./models/preprocess.onnx', import.meta.url).toString()),
        };
    }
    ...
    async execute(
        ...
        try {
            const templateSize = new this.CV.Size(TEMPLATE_SIZE, TEMPLATE_SIZE);
