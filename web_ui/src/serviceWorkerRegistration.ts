...
const scope = window.location.pathname.includes('/intel-admin') ? '/intel-admin/' : '/';
        ...
        window.addEventListener('load', () => {
            navigator.serviceWorker.getRegistration(scope).then((existingRegistration) => {
                if (existingRegistration) {
                    // A service worker is already registered or waiting
                    if (config && config.onSuccess) {
                        config.onSuccess(existingRegistration);
                    }
                } else {
