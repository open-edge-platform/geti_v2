...
def rerun_on_flaky_assert(error_msg_template=".*\\[flaky\\].*"):
    """
    Returns true if the failing assert in a test contains the tag [flaky]

    :Example:
    To re-run the test case up to three times if the accuracy is below the expected threshold:

    >>> from flaky import flaky
    ...
    ... @flaky(max_runs=3, rerun_filter=rerun_on_flaky_assert())
    ... def test_case():
    ...     assert model_accuracy() > 0.8,\
                "The analysis validation accuracy should be higher than 0.6 after training [flaky]"

    :param: error_msg_template: Template matching flaky assert messages.
    :return: whether to rerun the test (flaky) or not (hard failure)
    """

    def body(err, name, test, plugin):
        return re.match(error_msg_template, str(err[1])) is not None
...
class TestProject:
    def __init__(self, project, dataset, segmentation_model, circle_dataset, triangle_dataset) -> None:
        ...
        self.empty_dataset_item = next(item for item in dataset if len(item.annotation_scene.get_label_ids()) == 0)
