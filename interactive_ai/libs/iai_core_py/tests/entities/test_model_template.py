...
class CommonMethods:
    ...
    def check_model_attributes(model: ModelTemplate, expected_values: dict):
        ...
        assert model.model_status == expected_values.get("model_status", ModelTemplateDeprecationStatus.ACTIVE)
        ...
        for scenario in parameter_combinations:
            for parameters in scenario:
                ...
                assert dependency != unequal_dependency, (
                    f"Failed to check that Dependency instances with different {parameters} are unequal"
                )
        ...
        for scenario in parameter_combinations:
            for parameters in scenario:
                ...
                assert entry_points != unequal_entry_points, (
                    f"Failed to check that EntryPoints instances with different {parameters} are unequal"
                )
...
class TestModelTemplate:
    ...
    def optional_model_parameters(self):
        ...
        optional_parameters["model_status"] = ModelTemplateDeprecationStatus.ACTIVE
        optional_parameters["is_default_for_task"] = False
        return optional_parameters

    def test_model_template(self) -> None:
        model_templates = ModelTemplateList().get_all()
    ...
    def test_hyperparameters(self) -> None:
        ...
        # assert that hyperparameters are instantiated correctly and that they have no overrides
        assert isinstance(model_template.hyper_parameters, HyperParameterData)
        compare(model_template.hyper_parameters.parameter_overrides, {})
        assert not model_template.hyper_parameters.has_overrides

        # Specify overrides and test substituting the overrides
        new_max_batch_size = 1000
        model_template.hyper_parameters.parameter_overrides = {
            "learning_parameters": {
                "batch_size": {
                    "default_value": 20,
                    "max_value": new_max_batch_size,
                    "description": "Custom batch size",
                }
            }
        }
        model_template.hyper_parameters.substitute_parameter_overrides()
        assert model_template.hyper_parameters.has_overrides

        # Test that overrides are set in the parameter_overrides dict correctly
        compare(
            model_template.hyper_parameters.parameter_overrides["learning_parameters"]["batch_size"]["max_value"],
            new_max_batch_size,
        )

        # Test that the overrides also end up in the parameter data correctly
        compare(
            model_template.hyper_parameters.data["learning_parameters"]["batch_size"]["max_value"],
            new_max_batch_size,
        )

        # Test that passing an invalid metadata key raises a KeyError
        model_template.hyper_parameters.parameter_overrides.update(
            {"learning_parameters": {"batch_size": {"invalid_metadata_key": "SuperDuperNet"}}}
        )
        with pytest.raises(KeyError):
            model_template.hyper_parameters.substitute_parameter_overrides()

        # Test that passing an invalid parameter name raises a ValueError
        model_template.hyper_parameters.parameter_overrides["learning_parameters"].pop("batch_size")
        model_template.hyper_parameters.parameter_overrides.update(
            {"learning_parameters": {"invalid_parameter_name": {"default_value": "SuperDuperNet"}}}
        )
        with pytest.raises(ValueError):
            model_template.hyper_parameters.substitute_parameter_overrides()

    def test_remove_hyperparameter_values(self) -> None:
        # Arrange
        hyperparam_data: dict[str, Any] = {
            "learning_parameters": {
                "batch_size": {
                    "default_value": 20,
                    "value": 10,
        ...
        # Act
        HyperParameterData._HyperParameterData__remove_parameter_values_from_data(hyperparam_data)  # type: ignore

        # Assert
        assert "value" not in hyperparam_data["learning_parameters"]["batch_size"]
        assert "default_value" in hyperparam_data["learning_parameters"]["batch_size"]
        assert "ui_rules" in hyperparam_data["learning_parameters"]["batch_size"]
        assert "value" in hyperparam_data["learning_parameters"]["batch_size"]["ui_rules"]["rules"]
