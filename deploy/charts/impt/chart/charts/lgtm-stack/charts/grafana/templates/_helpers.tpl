...
app: {{ .Values.global.app_name_lgtm_stack }}
...
{{/*
Looks if there's an existing secret and reuses its admin-user value.
If not, it generates a new admin-user value and uses it.
*/}}
{{- define "grafana.user" -}}
{{- $secret := (lookup "v1" "Secret" (include "grafana.namespace" .) (include "grafana.fullname" .) ) }}
{{- if $secret }}
{{- index $secret "data" "admin-user" }}
{{- else }}
